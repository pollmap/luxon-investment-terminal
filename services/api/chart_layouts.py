...
def _normalized_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "visibility": {
            ...
            "metric_area": bool(visibility.get("metric_area", config.get("show_metric_area", True))),
...
def _layout_id(owner_key: str, name: str) -> str:
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"personal-fastgraphs:chart-layout:{owner_key}:{name}"))


def _manifest_path(owner_key: str, layout_id: str) -> Path:
    return _layout_dir(owner_key) / f"{layout_id}.json"


def _layout_dir(owner_key: str) -> Path:
    return Path(os.getenv("CHART_LAYOUT_DIR") or "storage/chart_layouts") / owner_key
