...
def load_chart_run(chart_run_id: str) -> dict[str, Any] | None:
    chart_run_id = _canonical_chart_run_id(chart_run_id)
    if postgres_enabled():
...
def _manifest_path(chart_run_id: str) -> Path:
    canonical_id = _canonical_chart_run_id(chart_run_id)
    root = _chart_run_dir().resolve()
    path = (root / f"{canonical_id}.json").resolve()
    if root not in path.parents:
        raise ValueError("chart_run_id resolves outside the chart run directory")
    return path
...
def _canonical_chart_run_id(chart_run_id: str) -> str:
    try:
        parsed = uuid.UUID(chart_run_id)
    except (AttributeError, TypeError, ValueError) as exc:
        raise ValueError("chart_run_id must be a canonical UUID") from exc
    canonical = str(parsed)
    if chart_run_id != canonical:
        raise ValueError("chart_run_id must be a canonical UUID")
    return canonical
...
def _with_evidence_summary(record: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        LOGGER.error("Failed to build chart evidence summary")
