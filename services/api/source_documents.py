...
RAW_SOURCE_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$")
SHA256_RE = re.compile(r"^[0-9A-Fa-f]{64}$")
KR_CACHE_TICKER_RE = re.compile(r"^[0-9]{6}\.(?:KS|KQ)$")
...
def _resolve_raw_content_hash(source_document_id: str) -> dict[str, Any] | None:
    ...
    if not RAW_SOURCE_RE.fullmatch(source) or not SHA256_RE.fullmatch(content_hash):
        return None
...
def _resolve_logical_id(source_document_id: str) -> dict[str, Any] | None:
    ...
    if source_document_id.startswith("derived:kr:"):
        parts = source_document_id.split(":")
        if len(parts) < 3 or _normalized_kr_cache_ticker(parts[2]) is None:
            return None
        source = "derived_metric"
    elif source_document_id.startswith("derived:"):
        source = "derived_metric"
    elif source_document_id.startswith("kr-cache:"):
        parts = source_document_id.split(":")
        if len(parts) < 2 or _normalized_kr_cache_ticker(parts[1]) is None:
            return None
...
def _resolve_kr_derived_valuation_input_id(source_document_id: str) -> dict[str, Any] | None:
    ...
    if len(parts) < 5 or parts[0] != "derived" or parts[1] != "kr" or parts[4] != "valuation-input":
        return None
    ticker = _normalized_kr_cache_ticker(parts[2])
    if ticker is None:
        return None
...
def _resolve_kr_cache_logical_id(source_document_id: str) -> dict[str, Any] | None:
    ...
    if len(parts) < 3 or parts[0] != "kr-cache":
        return None
    ticker = _normalized_kr_cache_ticker(parts[1])
    if ticker is None:
        return None
...
def _with_local_preview(record: dict[str, Any], *, include_preview: bool) -> dict[str, Any]:
    ...
    try:
        ...
        metadata["preview_error"] = "preview_unavailable"
...
def _find_file_by_hash(root: Path, content_hash: str) -> Path | None:
    ...
    for candidate in safe_root.rglob(f"*{short_hash}*"):
        if candidate.is_file() and _file_hash(candidate) == content_hash:
            return candidate
    checked = 0
...
def _find_kr_valuation_cache_file(ticker: str, fiscal_year: int | None) -> Path | None:
    ticker = _normalized_kr_cache_ticker(ticker)
    if ticker is None:
        return None
    ...
    candidates = sorted(
        candidate
        for candidate in (
            _resolved_file_within_root(cache_root, path)
            for path in cache_root.glob(f"{slug}-*-valuation-inputs.json")
        )
...
def _normalized_kr_cache_ticker(value: Any) -> str | None:
    ticker = str(value).upper()
    return ticker if KR_CACHE_TICKER_RE.fullmatch(ticker) else None


def _resolved_file_within_root(root: Path, candidate: Path) -> Path | None:
    try:
        resolved_root = root.resolve()
        resolved_candidate = candidate.resolve()
    except OSError:
        return None
    if resolved_root not in resolved_candidate.parents or not resolved_candidate.is_file():
        return None
...
def _raw_content_hash(source_document_id: str) -> str | None:
    if source_document_id.startswith(LOCAL_RAW_PREFIX):
        ...
        if (
            len(parts) == 3
            and RAW_SOURCE_RE.fullmatch(parts[1])
            and SHA256_RE.fullmatch(parts[2])
        ):
            return parts[2].lower()
    if SHA256_RE.fullmatch(source_document_id):
