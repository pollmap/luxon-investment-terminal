...
from services.api import chart_runs as chart_runs_module
from services.api import source_documents as source_documents_module
...
def test_source_document_resolver_rejects_invalid_raw_source_and_hash(monkeypatch, tmp_path):
    payload = b'{"private":"must not be selected by an invalid raw identifier"}'
    digest = hashlib.sha256(payload).hexdigest()
    raw_dir = tmp_path / "storage" / "raw" / "opendart"
    raw_dir.mkdir(parents=True)
    (raw_dir / f"document-{digest[:12]}.json").write_bytes(payload)
    monkeypatch.setenv("SOURCE_DOCUMENT_STORAGE_ROOT", str(tmp_path))

    invalid_ids = [
        "raw:opendart:*",
        f"raw:../opendart:{digest}",
        f"raw:opendart:{digest[:32]}",
    ]
    for source_document_id in invalid_ids:
        response = client.get(
            "/api/source-documents/resolve",
        ...
        assert data["preview_text"] is None


def test_source_document_resolver_verifies_full_file_hash(monkeypatch, tmp_path):
    claimed_hash = "a" * 64
    payload = b'{"private":"filename prefix is not hash proof"}'
    assert hashlib.sha256(payload).hexdigest() != claimed_hash
    raw_dir = tmp_path / "storage" / "raw" / "opendart"
    raw_dir.mkdir(parents=True)
    (raw_dir / f"document-{claimed_hash[:12]}.json").write_bytes(payload)
    monkeypatch.setenv("SOURCE_DOCUMENT_STORAGE_ROOT", str(tmp_path))

    response = client.get(
        "/api/source-documents/resolve",
        params={"source_document_id": f"raw:opendart:{claimed_hash}"},
    )
...
def test_source_document_resolver_rejects_unsafe_kr_cache_tickers(monkeypatch, tmp_path):
    cache_dir = tmp_path / "storage" / "cache" / "kr-valuation-inputs"
    cache_dir.mkdir(parents=True)
    (cache_dir / "005930_KS-2020-2025-valuation-inputs.json").write_text(
        json.dumps({"ticker": "005930.KS", "private": "must not be exposed"}),
        encoding="utf-8",
    )
    monkeypatch.setenv("SOURCE_DOCUMENT_STORAGE_ROOT", str(tmp_path))

    invalid_ids = [
        "kr-cache:*:2022:market-gap",
        "kr-cache:../../005930.KS:2022:market-gap",
        "derived:kr:**/005930.KS:2024:valuation-input",
        "derived:kr:005930[.]KS:2024:valuation-input",
    ]
    for source_document_id in invalid_ids:
        response = client.get(
...
def test_kr_cache_search_rejects_glob_candidate_outside_cache_root(monkeypatch, tmp_path):
    cache_dir = tmp_path / "storage" / "cache" / "kr-valuation-inputs"
    cache_dir.mkdir(parents=True)
    outside_path = tmp_path / "storage" / "imports" / "005930_KS-2020-2025-valuation-inputs.json"
    outside_path.parent.mkdir(parents=True)
    outside_path.write_text('{"private":"outside cache root"}', encoding="utf-8")
    monkeypatch.setenv("SOURCE_DOCUMENT_STORAGE_ROOT", str(tmp_path))

    original_glob = Path.glob

    def injected_glob(path: Path, pattern: str):
        if path.resolve() == cache_dir.resolve():
            return iter([outside_path])
        return original_glob(path, pattern)

    monkeypatch.setattr(Path, "glob", injected_glob)

    assert source_documents_module._find_kr_valuation_cache_file("005930.KS", 2022) is None
...
def test_chart_run_hides_evidence_summary_exception_from_response(monkeypatch, tmp_path, caplog):
    monkeypatch.setenv("CHART_CACHE_DIR", str(tmp_path / "charts"))
    monkeypatch.setenv("CHART_RUN_DIR", str(tmp_path / "chart-runs"))
    internal_detail = r"C:\sensitive\warehouse\tenant-data.json"

    def fail_summary(*_args, **_kwargs):
        raise RuntimeError(internal_detail)

    monkeypatch.setattr(chart_runs_module, "chart_source_summary", fail_summary)
    with caplog.at_level("ERROR", logger="services.api.chart_runs"):
        created = client.post(
            "/api/v1/charts/valuation-map/runs",
            json={"company_id": "AAPL", "metric": "adjusted_operating"},
        )

    assert created.status_code == 200
    run = created.json()["data"]
    assert run["evidence_summary"] == {"error": "chart_source_summary_failed"}
    assert internal_detail not in created.text
    assert "Failed to build chart evidence summary" in caplog.text
    assert internal_detail not in caplog.text

    manifest = client.get(f"/api/v1/charts/valuation-map/runs/{run['chart_run_id']}")
    assert manifest.status_code == 200
    assert manifest.json()["data"]["evidence_summary"] == {
        "error": "chart_source_summary_failed"
    }
    assert internal_detail not in manifest.text
...
def test_chart_run_rejects_noncanonical_and_traversal_ids(monkeypatch, tmp_path):
    monkeypatch.setenv("CHART_RUN_DIR", str(tmp_path / "chart-runs"))

    traversal = client.get(
        "/api/v1/charts/valuation-map/runs/..%5C..%5Cprivate-manifest"
    )
    noncanonical = client.get(
        "/api/v1/charts/valuation-map/runs/00000000000000000000000000000000"
    )

    assert traversal.status_code == 400
    assert traversal.json()["detail"] == "invalid chart run id"
    assert noncanonical.status_code == 400
    assert noncanonical.json()["detail"] == "invalid chart run id"
...
def test_chart_layouts_reject_path_like_owner_and_layout_ids(monkeypatch, tmp_path):
    monkeypatch.setenv("CHART_LAYOUT_DIR", str(tmp_path / "chart-layouts"))

    listed = client.get("/api/v1/chart-layouts?owner_key=..%5C..%5Cprivate")
    saved = client.post(
        "/api/v1/chart-layouts",
        json={
            "name": "unsafe owner",
    ...
    deleted = client.delete(
        "/api/v1/chart-layouts/..%5C..%5Cprivate?owner_key=default"
    )

    assert listed.status_code == 400
    assert listed.json()["detail"] == "invalid chart layout owner"
    assert saved.status_code == 400
    assert saved.json()["detail"] == "invalid chart layout owner"
    assert deleted.status_code == 400
    assert deleted.json()["detail"] == "invalid chart layout identifier"
